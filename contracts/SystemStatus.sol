...
contract SystemStatus is Owned, ISystemStatus {
    ...
    bytes32 public constant SECTION_SETTLE_TOKEN = "SettleToken";
    ...
    mapping(bytes32 => Suspension) public override settleTokenSuspension;
    ...
    function requireExchangeBetweenSynthsAllowed(bytes32 sourceCurrencyKey, bytes32 destinationCurrencyKey) external view override {
        // Synth exchange and transfer requires the system be active
        _internalRequireSystemActive();
    ...
    function getSettleTokenSuspensions(bytes32[] calldata sTokens)
        external
        view
        override
        returns (bool[] memory suspensions, uint256[] memory reasons)
    {
        suspensions = new bool[](sTokens.length);
        reasons = new uint256[](sTokens.length);

        for (uint i = 0; i < sTokens.length; i++) {
            suspensions[i] = settleTokenSuspension[sTokens[i]].suspended;
            reasons[i] = settleTokenSuspension[sTokens[i]].reason;
        }
    }
    /* ========== MUTATIVE FUNCTIONS ========== */
    ...
    function suspendSettleToken(bytes32 currencyKey, uint256 reason) external override {
        bytes32[] memory currencyKeys = new bytes32[](1);
        currencyKeys[0] = currencyKey;
        _internalSuspendSettleTokens(currencyKeys, reason);
    }

    function suspendSettleTokens(bytes32[] calldata currencyKeys, uint256 reason) external {
        _internalSuspendSettleTokens(currencyKeys, reason);
    }

    function resumeSettleToken(bytes32 currencyKey) external {
        bytes32[] memory currencyKeys = new bytes32[](1);
        currencyKeys[0] = currencyKey;
        _internalResumeSettleTokens(currencyKeys);
    }

    function resumeSettleTokens(bytes32[] calldata currencyKeys) external {
        _internalResumeSettleTokens(currencyKeys);
    }
    /* ========== INTERNAL FUNCTIONS ========== */
    ...
    function _internalSuspendSettleTokens(bytes32[] memory currencyKeys, uint256 reason) internal {
        _requireAccessToSuspend(SECTION_SETTLE_TOKEN);
        for (uint i = 0; i < currencyKeys.length; i++) {
            bytes32 currencyKey = currencyKeys[i];
            settleTokenSuspension[currencyKey].suspended = true;
            settleTokenSuspension[currencyKey].reason = uint248(reason);
            emit SynthSuspended(currencyKey, reason);
        }
    }
    ...
    function _internalResumeSettleTokens(bytes32[] memory currencyKeys) internal {
        _requireAccessToResume(SECTION_SETTLE_TOKEN);
        for (uint i = 0; i < currencyKeys.length; i++) {
            bytes32 currencyKey = currencyKeys[i];
            emit SynthResumed(currencyKey, uint256(synthSuspension[currencyKey].reason));
            delete settleTokenSuspension[currencyKey];
        }
    }
