...
//    bytes32 private constant CONTRACT_CHARM = "CharmToken";
    ...
    function _isSettleTokenInvalid(bytes32 settleTokenKey) internal view returns (bool) {
        //return systemStatus().isSettleTokenInvalid(settleTokenKey); // TODO: realization on systemStatus
        return false;
    }
    ...
    function _burnDTokens(
       ...
       address destAccount,
        uint amountOfDTokenToBurn
         ...
         bool isTradeTokenInvalid = _isSettleTokenInvalid(settleTokenKey);
         ...
         if (claimableProfits > 0) {
             if (isTradeTokenInvalid) {
             ...
             require(profitsClaimed > 0 && profitsClaimed < claimableProfits, "Profits claim amount must be greater than 0 and smaller than claimable profits.");
