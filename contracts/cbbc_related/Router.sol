...
//        if (msg.value > amountETH) TransferHelper.safeTransferETH(msg.sender, msg.value - amountETH);
    ...
    function removeLiquidityETH(
        ...
        (, uint256 liquidityToBurn) = issuerForLiquidityToken().calculateAmountOfSettleTokenToReceive(lTokenKey, liquidity);

        IERC20(address(liquidityToken)).transferFrom(msg.sender, address(issuerForLiquidityToken()), liquidityToBurn);
        // send liquidity to issuerForLiquidityToken()

        (amountETH, ) = issuerForLiquidityToken().burnLiquidityTokens(lTokenKey,  address(this), liquidity);

        require(amountETH >= amountETHMin, 'CBBC: INSUFFICIENT_SETTLEMENT_AMOUNT');
