...
//    using SafeDecimalMath for uint;
    ...
    function _issuedCTokens(bytes32 currencyKey) internal view returns (uint){
        address cTokenAddr = address(cTokens[currencyKey]);

        if(cTokenAddr == address(0)){
            return 0;
        }

        return IERC20(cTokenAddr).totalSupply();
    }
    ...
    function _cTokenKeysForSettleToken(bytes32 settleTokenKey) internal view returns (bytes32[] memory) {
        ...
        bytes32[] memory newCTokenKeys = new bytes32[](numCTokens);

        for (uint256 i = 0; i < numCTokens; i++) {
            newCTokenKeys[i] = cTokenKeys[i];
        }

        return newCTokenKeys;
    }
    ...
    function _claimableProfits(bytes32 settleTokenKey) internal view returns (uint) {
        if(settleTokenKey == CHARM){
            ...
            require(address(lToken) != address(0), "Liquidity Pool doesnot exist.");
            ...
            uint256 adjustFactor = 10**(18 - settleToken.decimals());

            uint256 settleTokensInPool = settleToken.balanceOf(address(lToken)) * adjustFactor;

            uint256 cTokensInPool = _totalIssuedCTokens(settleTokenKey);

            uint256 profits = settleTokensInPool * adjustFactor + initialSupply > totalSupply + cTokensInPool*2 ?
                settleTokensInPool * adjustFactor + initialSupply - totalSupply - cTokensInPool*2 : 0;

            return profits/adjustFactor;
        }
    }
...
//        cbbcAmount = settleAmount * (10**18) /(10 ** IERC20(cToken.settleTokenAddress()).decimals()) * (10**18) / cbbcPrice;
    ...
    */
        ...
        internal view returns (uint settleAmount)
    {
        ...
        settleAmount = cbbcAmount * cbbcPrice / (10**18) * (10000 - _leverage(cToken) * 6) / 10000 / 10**(18 - IERC20(cToken.settleTokenAddress()).decimals()); // transform to USDT * 10 s** 18; round fee = 6bp
    }
