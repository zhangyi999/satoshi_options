...
/*
interface IProxy {
    function target() external view returns (address); // TODO: why we need this?
}
*/
...
//    bytes32 private constant CONTRACT_EXRATES = "ExchangeRates";
    ...
    function _removeLToken(bytes32 currencyKey) internal {
        ...
        if (lTokenSupply > 0) { //TODO: what action will be taken if there is a supply?
...
//            _synthRedeemer.deprecate(IERC20(address(Proxyable(address(synthToRemove)).proxy())), rateToRedeem);
    ...
    function mintLiquidityTokens(
        ...
        _issueLTokens(token, destAccount, amountOfSettleToken);
    ...
    function burnLiquidityTokens(
        ...
        ) external lock onlyRouter override returns(uint256 amountOfSettleTokenToReceive){
        ILiquidityToken token = lTokens[currencyKey];
        ...
        if (settleTokenAddress == weth()) {
            amountOfLTokenToBurn = amountOfLTokenToBurn < settleTokenInPool * 10**(18 - IERC20(settleTokenAddress).decimals())/2 ? amountOfLTokenToBurn : settleTokenInPool * 10**(18 - IERC20(settleTokenAddress).decimals()) / 2;

            amountOfSettleTokenToReceive = amountOfLTokenToBurn * 10**(IERC20(settleTokenAddress).decimals()) / 10**18;

        } else {
            amountOfSettleTokenToReceive = settleTokenInPool * amountOfLTokenToBurn / totalSupplyOfLToken;
        }

        _burnLTokens(token, address(this), amountOfLTokenToBurn);
