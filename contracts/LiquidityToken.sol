...
contract LiquidityToken is BaseToken, ILiquidityToken {
    ...
    uint public isDuringSettingUp = 1;
    ...
    function issueInitialSupply(address destAccount, uint amount) external onlyOwner{
        require(settleTokenKey == bytes32("ETH") && isDuringSettingUp == 1, "CBBC: ONLY ETH and DURINNG SETTINGUP IS ALLOWED.");
        _internalIssue(destAccount, amount);
        initialSupply = amount;
        isDuringSettingUp = 0;
    }

    function issue(address account, uint amount) external override lock onlyIssuer {
        _internalIssue(account, amount);
    }

    function burn(address account, uint amount) external override lock onlyIssuerOrRouter returns(bool){
